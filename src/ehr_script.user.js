...
// @require https://cdn.jsdelivr.net/npm/@tomyail/happy-time@1.3.6/dist/index.umd.js
// ==/UserScript==
...
(function () {
  ...
  function colorLog(message, color, commit, color2) {
    color = color || 'black';
  ...
  setInterval(() => {
    ...
    if (
      ...
      colorLog(
        '__ehr_predict("10.7",185,"9.16","10.15")',
        'green',
        '预测剩余的考勤周期每天需要工作多久才能满足目标工作小时数(当前时间,考勤总工作数,考勤开始日期,考勤结束日期)',
        'red'
      );
      colorLog(
        '__ehr_set_absentHour("10.20",4)',
        'green',
        '设置 10.20 的请假时间为4 小时 (可选 0,4,8)',
        'red'
      );
      colorLog(
        '__ehr_set_workingHour("10.20",8)',
        'green',
        '强制设置 10.20 为工作日 (可选 0|8)',
        'red'
      );
      colorLog(
        '__ehr_averageHoursPerDays("9.16","10.15")',
        'green',
        '获取每天的平均工作小时数',
        'red'
      );
      ...
      btn.addEventListener('click', () => {
        window.happyTime
          ...
          .subscribe(
            (x) =>
              colorLog(
                window.happyTime.toReadableString(x, extra_config),
                'green'
              ),
            (x) => console.error(x),
            () => {
